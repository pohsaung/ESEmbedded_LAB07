...
int main(void)
{
	...
	//EXTI Configurations
	SET_BIT(EXTI_BASE+EXTI_IMR_OFFSET,0);  //0 mask not uesed
		
	SET_BIT(EXTI_BASE+EXTI_RTSR_OFFSET,0); //rising triger 1=enable
		
	CLEAR_BIT(EXTI_BASE+EXTI_FTSR_OFFSET,0); //falling trigrt 0=disable
		
	SET_BIT(EXTI_BASE+EXTI_PR_OFFSET,0); //clean PEDDING
...
void exti0_handler(void)
{
	...
	//clear pending
	SET_BIT(EXTI_BASE + EXTI_PR_OFFSET, 0);   // since it has been defind "weak" in vector table 
}
